...
/**
 * "Extracted from the symja project" The parent of all other two-dimensional
 * Plotters.
 * 
...
public abstract class AbstractPlotter2D extends JComponent implements Runnable {
	 ...
	 */
	protected static Color COLOR[] = new Color[] { Color.BLUE, Color.RED,
			Color.GREEN, Color.ORANGE, Color.MAGENTA, Color.PINK, Color.YELLOW,
			Color.CYAN };
	 ...
	 */
	protected static Font f = new Font("monospace", Font.PLAIN, 14);
	...
	/**
	 * Caches this object for future reuse, preventing memory leaks if the
	 * garbage collector gets sloppy on us.
	 */
	 ...
	 */
	protected abstract void paintPlots(Graphics2D g2d, int top, int height,
			int bottom, int left, int width, int right);
	...
	protected void setupText() {
		...
		ascent = (int) new TextLayout(Double.toString(yMax), f, frc)
				.getAscent();
	 ...
	 */
	protected void fillBackground(Graphics2D g2d, int xAxis, int yAxis,
	 ...
	 */
	protected void paintAxisLabels(Graphics2D g2d, int xAxis, int yAxis,
			int top, int bottom, int left, int right) {
		Shape yArrow = new Polygon(new int[] { yAxis, yAxis - ascent / 2,
				yAxis, yAxis + ascent / 2 }, new int[] { ascent, ascent * 2,
		...
		g2d.setPaint(new GradientPaint(right, 0, Color.WHITE, right + ascent,
				0, Color.BLACK));
		g2d.fill(new Polygon(new int[] { right, right + ascent, right,
				right + ascent / 3 }, new int[] { xAxis - ascent / 2, xAxis,
		...
		g2d.drawGlyphVector(yMaxText, yAxis - ascent - yMaxLength, top + ascent
				/ 2);
		g2d.drawGlyphVector(yMinText, yAxis - ascent - yMinLength, bottom
				+ ascent / 2);

		g2d.drawLine(left, xAxis, left, xAxis + ascent / 2);
		g2d.drawLine(right, xAxis, right, xAxis + ascent / 2);

		g2d.drawGlyphVector(xMaxText, right - xMaxLength / 2, xAxis + ascent
				* 2);
		g2d
				.drawGlyphVector(xMinText, left - xMinLength / 2, xAxis
						+ ascent * 2);
	}
