...
public class CheckString {
	...
	public GramErr GramCheck(String cadena) {
		...
		if (Equalsign) {
			...
			while (lector.hasMoreTokens()) {
				...
				PrevToken = aux3;
				// If the token is a function or pi I must translate it to
				// the matheclipse syntaxes
				if ((IsFunction(aux3)) /*| (aux3.equals("e"))*/
						| (aux3.equals("pi"))) {
					aux3 = f2F(aux3);
					if (IsArcTrigonometric(aux3) & (!Radianes)) {
						aux3 = "57.295779513*".concat(aux3);
					}
				}
				aux2 += aux3;
			}
	...
	public String P2C(String aux, String PrevToken) {
		...
		default: {
			if (IsFunction(PrevToken)) {
				if (IsTrigonometric(PrevToken) & (!IsArcTrigonometric(PrevToken))
						& (!Radianes)) {
					P.AddTerm((byte) 2);
					return "[Degree(";
				} else {
	 ...
	 * 
	 * @param aux
	 * @return True if the String is a trigonometric arc function
	 */
	public boolean IsArcTrigonometric(String aux) {
		if ((aux.equalsIgnoreCase("arcsin"))
				| (aux.equalsIgnoreCase("arccos"))
				| (aux.equalsIgnoreCase("arctan")))
			return true;
	...
	public void getVariables(String cadena, MaterialMethods Materiales,
		...
		//If e is stored in variables then store it in case Variables

		/*for (int ii = 1; i < CheckString.Var.getSize() + 1; ii++) {
			for (int jj = 1; i < CheckString.Var.Variables.size() + 1; jj++) {
				if (CheckString.Var.getVar(jj).equalsIgnoreCase("e")) {
					CheckString.CaseVariables.add("E");
